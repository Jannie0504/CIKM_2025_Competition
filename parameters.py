...
class DataTrainingArguments:
    ...
    test_file: Optional[str] = field(
        default=None, metadata={"help": "file containing the data to predict."}
    )
    eval_outputs: Optional[str] = field(
        default="eval.json", metadata={"help": "The output file name for predictions"}
    )
    outputs: Optional[str] = field(
        default="submit.json", metadata={"help": "The output file name for predictions"}
    )

    use_jaccard_qi: bool = field(
        default=False,
        metadata={"help": "QI에서만 자카드/컨테인먼트를 하이브리드로 사용"}
    )
    jac_weight: float = field(default=0.25)
    con_weight: float = field(default=0.25)
    prob_weight: float = field(default=0.5)   # 모델 확률 가중치
    hybrid_threshold: float = field(default=0.5)  # 최종 점수 임계값
